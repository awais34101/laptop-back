...
export const getReturnsStore2 = async (req, res) => {
  try {
    const page = Math.max(parseInt(req.query.page) || 1, 1);
    const limit = Math.min(Math.max(parseInt(req.query.limit) || 10, 1), 100);
    const skip = (page - 1) * limit;
    const [total, returns] = await Promise.all([
      ReturnStore2.countDocuments(),
      ReturnStore2.find()
        .sort({ date: -1 })
        .skip(skip)
        .limit(limit)
  ...
  try {
    ...
    await session.withTransaction(async () => {
      for (const { item, quantity, price } of items) {
        ...
        if (store2Doc) {
          store2Doc.remaining_quantity += quantity;
          await store2Doc.save({ session });
        } else {
          await Store2.create([{ item, remaining_quantity: quantity }], { session });
        }
      }
      createdReturn = new ReturnStore2({ items, customer, invoice_number });
...
export const deleteReturnStore2 = async (req, res) => {
  try {
    const doc = await ReturnStore2.findByIdAndDelete(req.params.id);
